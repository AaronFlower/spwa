...
spa.shell = (function () {
	...
	var 
		configMap = {
			anchor_schema_map : {		//目前只设置了两个状态。以后可能有扩充。
		...
		stateMap  = { 
			$container 			: null ,
			anchor_map 			: {},
			is_chat_retracted	: true	//在 stateMap 里面列出所有会用到的键是一种很好的做法，容易查找和查看。
		},
		jqueryMap = { },
	...
	copyAnchorMap = function () {
		return $.extend( true, { }, configMap.anchor_map );
	};
	// -------------------- END UTILITY METHODS ------------------------- 

	// -------------------- BGEIN DOM METHODS ---------------------------
	// Begin DOM method /setJqueryMap/
	setJqueryMap = function () {	//将聊天滑块的jQuery集合缓存到jqueryMap中。
	...
	// Eed DOM method /setJqueryMap/

	// Begin DOM method /toggleChat/	//下面其实写的是一个API文档。
	...
	toggleChat = function ( do_extend , callback) {
		var 
			px_chat_ht 	= jqueryMap.$chat.height(),	//已经缓存
	...
	changeAnchorPart = function ( arg_map ) {
		...
		for( key_name in arg_map ){
			if( arg_map.hasOwnProperty( key_name ) ) {
				...
				// update matching dependent key value 
...
/*		//test toggle  需求5：测试， 不需要单击事件也能测试。
		...
		// This is done /after/ all feature modules are configured and initialiazed, otherwise
		// they will not be ready to handle the trigger event, which is used to ensure the anchor is 
		// considered on-load.
		$( window )
			.bind( 'hashchange' , onHashchange)
			.trigger( 'hashchange' );		//绑定并立即触发。
	};
